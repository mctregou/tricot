...
class StepActivity : BaseActivity() {
    ...
    override fun onResume() {
        ...
        settings.setOnClickListener {
            ...
            dialog.delete_step.setOnClickListener {
                ...
                val confirmationDialog = Utils().showDialog(
                    this,
                    R.string.warning,
                    R.string.step_delete_confirmation,
                    View.OnClickListener {
                        RealmManager().open()
                        RealmManager().createRuleDao().removeAllById(step?.stitches?.toList())
    ...
    private fun getRules() {
        RealmManager().open()
        rules =
            ArrayList(RealmManager().createRuleDao().loadByIds(step?.stitches?.toList()).toList())
        RealmManager().close()

        rules_recyclerview.layoutManager =
    ...
    private fun getReductions() {
        ...
        if (reduction != null) {
            reductions_description.text = String.format(
                getString(R.string.frequence_description),
                reduction?.frequency.toString(),
                reduction?.offsetRank.toString()
            )

            RealmManager().open()
            val reductionItemsDB =
                RealmManager().createReductionItemDao().loadByReductionId(reduction?.id!!)
            RealmManager().close()
        ...
        reductions_recyclerview.layoutManager =
            LinearLayoutManager(this)
    ...
    private fun addRule() {
        ...
        dialog.window?.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
        dialog.stitches_recyclerview.layoutManager =
            LinearLayoutManager(this)
    ...
    private fun addReduction() {
        ...
        reductionDialog?.reduction_items_recyclerview?.layoutManager =
            LinearLayoutManager(this)
    ...
    private fun addReductionItem() {
        ...
        val reductionItem = ReductionItem(
            0,
            null,
            index,
            Integer.parseInt(reductionDialog?.edittext_times?.text.toString()),
            Integer.parseInt(reductionDialog?.edittext_numberOfMesh?.text.toString()),
            reductionDialog?.edittext_side?.text.toString()
        )
    ...
    private fun plus() {
        ...
        EventBus.getDefault()
            .post(UpdateNotification(true, intent.getIntExtra(Constants.PROJECT_ID, 0), step?.id!!))
    }
    ...
    private fun checkRule() {
        ...
        current_rules_recyclerview.layoutManager =
            LinearLayoutManager(this)
        ...
        if (reduction != null
            ...
            for (reductionItem in reductionItems) {
                if (position >= (count + reductionItem.times)) {
                    ...
                    current_reduction_description.text = String.format(
                        "• %d fois %d mailles du côté %s",
                        reductionItem.times,
                        reductionItem.numberOfMesh,
                        reductionItem.side
                    )
                    break
                }
